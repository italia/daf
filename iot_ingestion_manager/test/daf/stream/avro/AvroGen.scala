...
object AvroGen {
  ...
  val event = for {
    ...
    .setSubtype { subType getOrElse "" }
    .setEventAnnotation { eventAnnotation getOrElse "" }
    .setSource(source)
    .setLatitude { location.map { _.getLatitude } getOrElse 0.0 }
    .setLongitude { location.map { _.getLongitude } getOrElse 0.0 }
    .setPayload(payload)
