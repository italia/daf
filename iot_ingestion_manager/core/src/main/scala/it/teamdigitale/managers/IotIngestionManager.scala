...
object IotIngestionManager  {
  ...
  def run(
    ...
    streamingContext = new StreamingContext(sc.sparkContext, Seconds(1))
    ...
    */
  def run(sc: SparkSession, kuduContext: KuduContext, configuration: Config): Unit  = {
    ...
    run(sc: SparkSession, kuduContext: KuduContext, kafkaconfig, kuduConfig, hdfsConfig)
