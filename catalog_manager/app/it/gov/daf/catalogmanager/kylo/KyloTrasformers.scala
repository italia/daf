...
object KyloTrasformers {
  ...
  def feedTrasform(metaCatalog: MetaCatalog, template :JsValue, templates : List[JsObject]): Reads[JsObject] = __.json.update(
       ((__ \ 'feedName).json.put(JsString(metaCatalog.dcatapit.name)) and
        (__ \ 'description).json.put(JsString(metaCatalog.dcatapit.name)) and
         ...
         ((__ \ 'table) \ 'tableSchema).json.put(Json.obj("name" -> (metaCatalog.operational.group_own + "_o_" + metaCatalog.dcatapit.name)
