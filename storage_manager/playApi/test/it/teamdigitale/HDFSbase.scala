...
abstract class HDFSbase extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  val (testDataPath, confPath) = {
    val testDataPath = s"${PathUtils.getTestDir(classOf[HDFSbase]).getCanonicalPath}/MiniCluster"
  ...
  private def writeDf(): Unit = {
    ...
    Logger.info(s"TestDataPath ${testDataPath.toJava.getAbsolutePath}")
    Logger.info(s"ConfPath ${confPath.toJava.getAbsolutePath}")
