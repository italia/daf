...
class DafUriConverter(
  ...
  private val logicalUri = dsType match {
    case Standard =>
      s"daf://dataset/standard/$theme__$subTheme/$dsName"
    case Ordinary =>
      s"daf://dataset/$organization/$theme__$subTheme/$dsName"
    case OpenData =>
  ...
  private val physicalUri = dsType match {
    case Standard =>
      s"/daf/standard/$theme__$subTheme/$dsName"
    case Ordinary =>
      s"/daf/ordinary/$organization/$theme__$subTheme/$dsName"
    case OpenData =>
      s"/daf/opendata/$dsName"
  }
