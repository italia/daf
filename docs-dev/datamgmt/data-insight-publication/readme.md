...
In order to allow the publication of insights arising from data analysis activities, the DAF platform provide a specific Web Application: the *Data Portal* is a web based user interface for the DAF, giving users the capabilities to access to DAF functionalities and data products. The Data Portal will expand the typical features of the popular data catalogues (e.i. dati.gov.it) with the following:
