...
lazy val catalog_manager = (project in file("catalog_manager")).
  enablePlugins(PlayScala, ApiFirstCore, ApiFirstPlayScalaCodeGenerator, ApiFirstSwaggerParser).
  settings(
    commonSettings
  ).enablePlugins(AutomateHeaderPlugin, DockerPlugin)

lazy val schema_manager = (project in file("schema_manager")).
  enablePlugins(PlayScala, ApiFirstCore, ApiFirstPlayScalaCodeGenerator, ApiFirstSwaggerParser).
  settings(
    commonSettings
  ).enablePlugins(AutomateHeaderPlugin, DockerPlugin)

lazy val dataset_manager = (project in file("dataset_manager")).
  enablePlugins(PlayScala, ApiFirstCore, ApiFirstPlayScalaCodeGenerator, ApiFirstSwaggerParser).
  settings(
    commonSettings
  ).enablePlugins(AutomateHeaderPlugin, DockerPlugin)

lazy val storage_manager = (project in file("storage_manager")).
  enablePlugins(PlayScala, ApiFirstCore, ApiFirstPlayScalaCodeGenerator, ApiFirstSwaggerParser).
  settings(
    commonSettings
  ).enablePlugins(AutomateHeaderPlugin, DockerPlugin)

lazy val root = (project in file(".")).aggregate(catalog_manager, schema_manager, dataset_manager, storage_manager)
